...
def load_all(csv_globs):
    """Load all CSV files and return list of arrays, labels, and class names."""
    X, y, classes = [], [], []
    for csvname in csv_globs:
        ...
        required_cols = {'gesture','rep','t_ms','gx','gy','gz','ax','ay','az'}
...
def pad_or_trim(arr, target_len):
    """Pad with zeros (neutral) or trim to target length."""
    n = arr.shape[0]
    if n == target_len:
        return arr
    if n < target_len:
...
def build_dataset(X_list, y_list, classes, target_len):
    """Convert list of arrays and labels into arrays for training."""
...
def normalize_train(X_train, X_val):
    """Compute safe per-channel mean/std from train set."""
    mean = X_train.mean(axis=(0,1), keepdims=True)
    std = X_train.std(axis=(0,1), keepdims=True)
    std[std < 1e-6] = 1.0  # avoid div by 0
    return (X_train - mean)/std, (X_val - mean)/std, mean, std

def make_model(input_shape, num_classes):
    """Small Conv1D network for time-series gesture classification."""
...
def convert_to_tflite(model, X_train):
    """Convert model to float32 and INT8 TFLite models."""
    os.makedirs(MODEL_DIR, exist_ok=True)
    # Float32 TFLite
    tflite_model = tf.lite.TFLiteConverter.from_keras_model(model).convert()
    open(MODEL_TFLITE, "wb").write(tflite_model)
    print(f"Saved float32 TFLite to {MODEL_TFLITE}")
...
def main():
    ...
    # Save label mapping + normalization
    mapping = {"classes": classes, "mean": mean.flatten().tolist(), "std": std.flatten().tolist(), "timesteps": TIMESTEPS}
